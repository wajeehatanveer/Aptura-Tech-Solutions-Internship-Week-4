...
# AUTOMATION
    ...
    st.markdown("""
    <div class="hero">
        <div class="hero-title">Automated Ticket Processing</div>
        <p class="hero-text">
            Automatically monitor tickets, identify issues that
            require attention, and generate processing reports.
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.info(
        "This automation reads ticket data, analyzes tickets, "
        "identifies tickets requiring attention, and generates a CSV report. "
        "It does not modify or delete ticket records."
    )
    ...
    if st.button(
        ...
        type="primary",
        use_container_width=True,
        key="run_automation_button"
    ):

        try:

            with st.spinner(
                "Processing tickets and generating report..."
            ):

                processing_result = (
                    ticket_processor.process_tickets()
                )
            ...
            st.session_state.automation_result = {
                ...
                "tickets": processing_result["tickets"]
            }
            ...
            logger.info(
                "Automation completed successfully from UI."
            )

        except Exception as error:

            logger.exception(
                f"Automation failed from UI: {error}"
            )
    ...
    if "automation_result" in st.session_state:
        ...
        st.subheader("Latest Automation Run")
        ...
        st.write(
            f"**Last Run:** {result['processed_at']}"
        )
        ...
        st.subheader("📊 Automation Report Preview")
        ...
        for ticket in result["tickets"]:
            ...
            report_preview.append({
                ...
                "Assigned To": ticket.get("assigned_to", "") or "Unassigned",
        ...
        if report_preview:
            ...
            st.info(
                "No ticket records were available for the report."
            )
        ...
        try:
            ...
            st.download_button(
                label="⬇️ Download Automation Report (CSV)",
                data=report_data,
            ...
            logger.error(
                f"Unable to read automation report: {error}"
            )

            st.error(
                "Unable to load the generated report."
            )
        ...
        st.info(
            ...
            "Click 'Run Automation Now' to process the tickets."
        )